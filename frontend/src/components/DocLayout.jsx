...
// 导入通用UI配置（若使用uiConfig.json，需确保路径正确）
import uiConfig from './Docs/json/uiConfig.json';

// 若未使用uiConfig.json，可保留此基础配置（若已导入uiConfig则删除）
const UI_CONFIG = {
  colors: {
    info: '#0ea5e9',
    warning: '#f59e0b',
...
const DocLayout = ({ children, headings = [] }) => {
  ...
  const [isMobile, setIsMobile] = useState(false); // 新增：判断是否为移动端
  const headingRefs = useRef({});

  // 新增：监听屏幕尺寸，判断是否为移动端（768px为断点）
  useEffect(() => {
    const checkIsMobile = () => {
      ...
      <div style={{
        ...
        <div style={{
          ...
          {typeof children === 'string' ? (
            <span style={{ whiteSpace: 'normal', wordBreak: 'break-word' }}>{children}</span>
          ) : (
